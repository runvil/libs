...
Runvil Libraries (`runvil-libs`) is a monorepo of modular, reusable libraries
written in Go for the **Runvil ecosystem**. It hosts the shared building
blocks that power the [Runvil Framework](https://github.com/runvil/runvil-framework)
...
- Module root `github.com/runvil/runvil-libs` with `core/` and `term/` packages.
...
## 4. Scope — Initial Phase: CLI Building Blocks

To support the framework's CLI ecosystem, `runvil-libs` will provide the
